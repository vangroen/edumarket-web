...
const AgentAddModal = ({ onClose, onSave, catalogs }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    documentNumber: '',
    idDocumentType: '',
    // Aquí puedes añadir campos específicos para el Agente en el futuro
    // Por ejemplo: commissionRate: 0,
  });

  // Efecto para establecer el valor por defecto del select
  useEffect(() => {
    if (catalogs.documentTypes && catalogs.documentTypes.length > 0) {
      setFormData(prev => ({
        ...prev,
        idDocumentType: '', // Forzamos a que elija una opción
      }));
    }
  }, [catalogs]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
      <div className="bg-dark-surface rounded-lg shadow-2xl w-full max-w-2xl p-8 m-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Aquí podrías añadir una sección para "Datos de Agente" si es necesario */}
