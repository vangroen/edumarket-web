...
const CatalogCrud = ({ catalogInfo, onBack }) => {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      // Cargamos los datos principales del catálogo
      const data = await fetchData(endpoint);
      setItems(data);

      // Si hay campos de tipo 'select', cargamos sus opciones
  ...
  const handleSave = async (itemToSave) => {
    try {
      ...
      fields.forEach(field => {
        // Convertimos a número si es un ID de un select
  ...
  const handleCloseDeleteModal = () => { /* ... (sin cambios) ... */ };
