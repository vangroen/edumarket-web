    ...
    return (
        <div className="bg-dark-bg/50 p-3 rounded-lg grid grid-cols-[auto,1fr,auto,auto,auto] items-center gap-x-4">
            ...
            <div className="flex items-center gap-2 overflow-hidden">
                {/* --- CAMBIO AQUÍ: Clases para truncar el texto --- */}
...
const CourseAddModal = ({ onClose, onSave, courseTypes, modalities, allInstitutions }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.institutions.length === 0) {
      alert("Por favor, añade al menos una institución.");
      return;
    }
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
        {/* --- CAMBIO AQUÍ: Ancho reducido a max-w-3xl --- */}
        <form onSubmit={handleSubmit} className="bg-dark-surface rounded-lg shadow-2xl w-full max-w-3xl flex flex-col max-h-full">
          ...
          <div className="overflow-y-auto p-8 space-y-6">
            <fieldset>
                ...
                <div className="space-y-4">
                    <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Ej: Desarrollo Web Full Stack" className="w-full bg-dark-bg border border-dark-border rounded-lg py-2 px-4 text-dark-text-primary" required />
            ...
            <fieldset>
                ...
                <p className="text-sm text-dark-text-secondary mb-4">Busca y añade las instituciones que imparten este curso.</p>
          ...
          <div className="flex-shrink-0 flex justify-end gap-4 p-8 pt-6">
            ...
            <button type="submit" className="px-6 py-2 bg-brand-accent text-white rounded-lg hover:bg-blue-600 font-semibold shadow transition-colors">Guardar Curso</button>
          </div>
        </form>
      </div>
